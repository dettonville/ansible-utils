...
def format_description(desc: str | list[str]) -> str:
    """Format description list or string into single paragraph with
    line breaks."""
    ...
    if options:
        ...
        for opt_name, opt in options.items():
            ...
            if choices:
                ...
                choices_col.extend([f"- `{c}`" for c in choices])
...
def main():
    ...
    parser.add_argument(
        ...
        "--force",
        action="store_true",
        help="force update of all plugin documentation",
    )
    parser.add_argument(
        "-L",
        "--loglevel",
    ...
    with tempfile.TemporaryDirectory(prefix="ansible_doc_") as temp_dir:
        ...
        # Find plugin files in plugins/modules, plugins/lookup, plugins/filter
        plugin_files = []
