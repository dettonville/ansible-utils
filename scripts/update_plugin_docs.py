...
def generate_markdown(
    ...
    # Sanitize collection name for the GitHub Pages repository URL slug
    repo_name_slug = name.replace("_", "-")
    doc_type_suffix = "module" if plugin_type == "module" else plugin_type

    github_pages_url = (
        f"https://{namespace}.github.io/ansible-{repo_name_slug}/"
        f"{module_name}_{doc_type_suffix}.html"
    )

    # Preliminary Reference Link
    md = [
        f"## module > {module_name}\n",
        f"> **Note:** The compiled HTML documentation for "
        f"this {plugin_type} is available at: "
        f"[{github_pages_url}]({github_pages_url})\n",
    ]
