...
LIST_ENTRIES = frozenset(
    ("supershipit", "maintainers", "labels", "keywords", "notify", "ignore")
)
    ...
    if not filename.startswith("plugins/filter/"):
        ...
        for maintainer in maintainers:
            ...
            if maintainer is not None and maintainer not in all_maintainers:
                msg = (
                    "Author %s not mentioned as active or inactive maintainer for %s (mentioned are: %s)"
    ...
    if not all_maintainers and not should_have_no_maintainer:
        print(
            ...
            % (
                ...
                0,
                0,
                "No (active or inactive) maintainer mentioned for %s" % filename,
            )
        )
    if all_maintainers and should_have_no_maintainer:
        print(
            ...
            % (
                ...
                0,
                0,
                "Please remove %s from the ignore list of %s" % (filename, sys.argv[0]),
            )
        )
