...
# No need for explicit import of main here; it's imported via patches in
# conftest.py or tests.
# Assuming conftest.py with mock_ntlm_uri_imports fixture is in place to
# handle global import checks.
...
def mock_module():
    ...
    mock_fail_json = Mock()
    with patch(
        "ansible_collections.dettonville.utils.plugins."
        "modules.ntlm_uri.AnsibleModule"
...
def test_post_raw_body(mock_requests, mock_module):
    """Test POST with raw body (string). Note: Module bug - uses json= even
    for raw."""
    raw_body = "raw data"
    # noinspection HttpUrlsUsage
    mock_module[0].params = {
...
def test_check_mode_get(mock_module):
    """Test check mode for GET - should make real call (as per
    module logic)."""
    # noinspection HttpUrlsUsage
    mock_module[0].params = {
        "body": None,
    ...
    with patch(
        ...
        # In check_mode, for GET, it should still call requests (as per
        # code: only mocks if method != GET)
        mock_requests.request.assert_called_once()
        # noinspection HttpUrlsUsage
        mock_module[1].assert_called_once_with(
            changed=False,
...
def test_check_mode_post(mock_module):
    """Test check mode for POST - should mock response without calling
    requests."""
    # noinspection HttpUrlsUsage
    mock_module[0].params = {
        "body": None,
    ...
    # Check the mocked exit (note: always sets json in check_mode, even if
    # return_content=False)
    # noinspection HttpUrlsUsage
    mock_module[1].assert_called_once_with(
        changed=False,
