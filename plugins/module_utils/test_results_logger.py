...
try:
    ...
    class TestCase(object):
        ...
        def add_failure_info(self, message=None,
                             output=None, failure_type=None):
        ...
        def is_failure(self):
            ...
            return sum(
                1 for f in self.failures if f["message"] or f["output"]) > 0

        def is_error(self):
            """returns true if this test case is an error"""
            return sum(
                1 for e in self.errors if e["message"] or e["output"]) > 0
    ...
    class TestSuite(object):
        ...
        def __setitem__(self, test_case_id: str, test_case: TestCase):
            log_prefix = "%s.__setitem__(%s):" % (
                self.__class__.__name__, test_case_id)
    ...
    class TestSuitesBase(object):
        ...
        def __init__(self, test_suites=None):
            ...
            logging.info(
                "%s unsupported test_suites type: %s", log_prefix, type(
                    test_suites)
            )
            raise ValueError(
                f"unsupported test_suites type: {
                    type(test_suites)}")
    ...
    class TestSuites(TestSuitesBase):
        ...
        def __init__(self, test_suites=None):
            ...
            if test_suites:
                ...
                logging.info(
                    "%s setting test_suites to empty dict",
                    log_prefix)
                # self.test_suites = OrderedDict()
                self.test_suites = dict()
...
def to_xml_report_file(file_descriptor, test_suites,
                       prettyprint=True, encoding=None):
    """
...
class TestResultsLogger:
    def __init__(self, module):
        ...
        self.loglevel = self.module.params.get(
            "logging_level") or _LOGLEVEL_DEFAULT
        ...
        self.test_junit_report_file = (
            self.module.params.get(
                "test_junit_report_file") or _LOGLEVEL_DEFAULT
        )
    ...
    def update_test_results(self, test_results: dict) -> dict[bool, bool]:
        ...
        if test_results:
            ...
            for test_suite_id, test_suite_data in test_results["test_suites"].items(
                ...
                self.log.debug(
                    "%s update self.test_suites with test_suite",
                    log_prefix)
                self.test_suites.update(test_suite)
