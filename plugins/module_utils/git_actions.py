...
class Git:
    ...
    def clone(self, shallow=True, bare=False, reference=None, refspec=None) -> dict:
        ...
        self.log.debug("%s(%s): rc=%d", log_prefix, self.repo_dir, rc)
        self.log.debug("%s(%s): stdout=%s", log_prefix, self.repo_dir, stdout)
        self.log.debug("%s(%s): stderr=%s", log_prefix, self.repo_dir, stderr)
        ...
        if stdout:
            result.update({"message": stdout, "git.clone": stdout, "changed": True})
    ...
    def pull(self) -> dict:
        ...
        if rc == 0:
            if stdout:
                result.update({"message": stdout, "git.pull": stdout, "changed": True})
        else:
            FailingMessage(self.module, rc, command, stdout, stderr)
    ...
    def add(self, add_files=None) -> dict:
        ...
        if rc == 0:
            if stdout:
                result.update({"message": stdout, "git.add": stdout, "changed": True})
        else:
            FailingMessage(self.module, rc, command, stdout, stderr)
    ...
    def commit(self, comment="ansible update") -> dict:
        ...
        if rc == 0:
            if stdout:
                result.update(
                    {"message": stdout, "git.commit": stdout, "changed": True}
                )
        else:
            FailingMessage(self.module, rc, command, stdout, stderr)
