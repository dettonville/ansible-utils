...
short_description: Print sanitized debug statements with automated regex redactions
...
options:
  msg:
    description: A customized string message to print out. Mutually exclusive with the C(var) option.
    type: str
    default: "Hello world!"
  var:
    description: A variable name string or complex data structure to evaluate and print out. Mutually exclusive with the C(msg) option.
    type: raw
  verbosity:
    description: A number that controls when the debug module will run based on the defined verbosity level configurations.
    type: int
    default: 0
  key_patterns:
    description: List of regular expressions used to identify and redact sensitive keys.
    type: list
    elements: str
    default: ['(?i).*vault.*', '(?i).*token.*', '(?i).*password.*', '(?i).*key.*', '(?i).*ssh.*']
  additional_key_patterns:
    description: Additional collection of regular expressions to append to the default evaluation arrays.
    type: list
    elements: str
"""
...
RETURN = """
msg:
  description: The sanitized string output message when using the msg option.
  type: str
  returned: always
var:
  description: The sanitized dictionary object block structure when using the var option.
  type: raw
  returned: when var is specified
failed:
  description: Tracking status flag checking execution runtime issues.
  type: bool
  returned: always
"""
...
def main():
    ...
    module.exit_json(changed=False, msg="Bypassed by action plugin framework.")
