...
def _parse_certificate(cert_content, module):
    ...
    try:
        ...
        try:
            ...
            module.fail_json(
                msg=f"Could not parse certificate. Must be PEM or DER format. Error: {str(e)}"
            )
...
def main():
    ...
    if issuer_path:
        ...
        try:
            ...
            issuer_cert_openssl = crypto.load_certificate(
            ...
            try:
                cert_openssl = crypto.load_certificate(
                    crypto.FILETYPE_DER, cert_content
                )
                issuer_cert_openssl = crypto.load_certificate(
                    crypto.FILETYPE_DER, issuer_content
                )
            except crypto.Error as e:
                module.fail_json(
        ...
        try:
            ...
            result["valid"] = False
            result["failed"] = True
            result["msg"] = f"Certificate signature validation failed: {str(e)}"
    ...
    if module.params.get("common_name"):
        ...
        if common_name is None or common_name != module.params["common_name"]:
            ...
            result["failed"] = True
            result["msg"] = (
                f"Common name mismatch. Expected '{module.params['common_name']}', found '{common_name}'"
            )
            module.fail_json(**result)
    ...
    if module.params.get("organization"):
        ...
        if organization is None or organization != module.params["organization"]:
            ...
            result["failed"] = True
            result["msg"] = (
                f"Organization mismatch. Expected '{module.params['organization']}', found '{organization}'"
            )
            module.fail_json(**result)
    ...
    if module.params.get("organizational_unit"):
        ...
        if (
            ...
            or organizational_unit != module.params["organizational_unit"]
        ):
            result["valid"] = False
            result["failed"] = True
            result["msg"] = (
                f"Organizational unit mismatch. Expected '{module.params['organizational_unit']}', found '{organizational_unit}'"
            )
            module.fail_json(**result)
    ...
    if module.params.get("key_algo"):
        if (
            actual_key_type is None
            or actual_key_type.upper() != module.params["key_algo"].upper()
        ):
            result["valid"] = False
            result["failed"] = True
            result["msg"] = (
