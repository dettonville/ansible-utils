...
| :--- | :--- | :--- |
| **action**<br>`str` | Default: `"pacp"`<br>Choices:<br>- `clone`<br>- `pull`<br>- `pacp`<br>- `acp` | Git operation to perform - 'clone', 'pull', 'pacp'(Pull + Add + Commit + Push), or 'acp' (Add + Commit + Push) |
...
| **comment**<br>`str` |  | Git commit comment. Same as C(git commit -m). Required if action performs commit (e.g., if action in ['pacp', 'acp']) |
...
| **mode**<br>`str` | Choices:<br>- `ssh`<br>- `https`<br>- `local` | Git operations are performend either over ssh, https or local. Same as C(git@git...) or C(https://user:token@git...). If not specified, the mode will be set to the scheme implied by the URL. |
...
> MODULE dettonville.utils.git_pacp (/var/folders/w6/3rcdpp211v5cxml6vg45ww3r0000gn/T/ansible_doc_cowvjbd9/ansible_collections/dettonville/utils/plugins/modules/git_pacp.py)
